...
export async function POST(request: NextRequest) {
  try {
    ...
    const tipo = request.nextUrl.searchParams.get("tipo") || "bovinos"
    ...
    const worksheet = workbook.addWorksheet("Lista ICA")
    ...
    const numericColumns = [5, 6, 7, 8, 9] // Machos, Hembras, Total Animales, Kilos, Total
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename=lista_ica_${tipo}_${new Date().toISOString().split("T")[0]}.xlsx`,
      },
    })
  } catch (error) {
