...
export async function POST(request: NextRequest) {
  try {
    ...
    doc.setFontSize(14)
    doc.text("Báscula Diaria - Porcinos", doc.internal.pageSize.width / 2, 30, { align: "center" })
    ...
    return new NextResponse(Buffer.from(pdfBuffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename=bascula_diaria_${new Date().toISOString().split("T")[0]}.pdf`,
      },
    })
  } catch (error) {
