...
export function BasculaBovinosClient() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const apiUrl = usingAltApi ? `/api/bascula-diaria-bovinos-alt` : `/api/bascula-diaria-bovinos`
        ...
        if (usingAltApi || err.message !== "Intentando con API alternativa...") {
          ...
          console.error("Error al cargar datos de Báscula Diaria Bovinos:", err)
        }
      } finally {
  ...
  const exportToCsv = useCallback(() => {
    try {
      ...
      let title = "Báscula Diaria - Bovinos"
      if (dateRange.from && dateRange.to) {
      ...
      link.setAttribute("download", `bascula_diaria_bovinos_${new Date().toISOString().split("T")[0]}.csv`)
  ...
  const exportToExcel = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/export/bascula-diaria-bovinos/excel`, {
        method: "POST",
        headers: {
      ...
      const link = document.createElement("a")
      link.href = url
      link.setAttribute("download", `bascula_diaria_bovinos_${new Date().toISOString().split("T")[0]}.xlsx`)
      document.body.appendChild(link)
      link.click()
  ...
  const exportToPdf = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/export/bascula-diaria-bovinos/pdf`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const link = document.createElement("a")
      link.href = url
      link.setAttribute("download", `bascula_diaria_bovinos_${new Date().toISOString().split("T")[0]}.pdf`)
      document.body.appendChild(link)
      link.click()
      link.parentNode?.removeChild(link)
      window.URL.revokeObjectURL(url)
