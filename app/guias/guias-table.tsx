  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (guia) =>
          ...
          (guia.fecha_documento && guia.fecha_documento.toLowerCase().includes(term)),
      )
    }
    ...
    if (dateRange.from || dateRange.to) {
      result = result.filter((guia) => {
        if (!guia.fecha_documento) return true

        // Convertir la fecha de la guía a formato DD/MM/YYYY para comparación
        const guiaDateParts = guia.fecha_documento.split("/")
        if (guiaDateParts.length !== 3) return true

        const guiaDate = new Date(
          Number.parseInt(guiaDateParts[2]), // año
          Number.parseInt(guiaDateParts[1]) - 1, // mes (0-11)
          Number.parseInt(guiaDateParts[0]), // día
        )
            ...
            {filteredGuias.length > 0 ? (
              filteredGuias.map((guia) => (
                <TableRow key={guia.id}>
                  ...
                  <TableCell>{guia.fecha_documento}</TableCell>
