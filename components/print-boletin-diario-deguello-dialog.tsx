...
interface PrintBoletinDiarioDeguelloDialogProps {
  isOpen: boolean
  onClose: () => void
  data: any[]
...
export function PrintBoletinDiarioDeguelloDialog({
  isOpen,
  onClose,
  data,
  totalDeguello,
  ...
  totalHembras,
  boletinNumber,
}: PrintBoletinDiarioDeguelloDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Imprimir Boletín Diario Deguello G/M</DialogTitle>
        </DialogHeader>
        <div className="py-4">
          <p>Se generará un PDF con el formato de boletín diario de deguello para ganado mayor.</p>
          <p className="mt-2">
